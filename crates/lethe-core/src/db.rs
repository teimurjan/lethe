...
CREATE TABLE IF NOT EXISTS entry_embeddings (
    -- Rust-side embedding storage (Python writes embeddings.npz so it
    -- ignores this table). Additive: a Python-written DB still works
    -- under Rust, just with no embeddings until reindex.
...
impl MemoryDb {
    ...
    /// Bulk write embeddings as a single transactional batch. Use this
    /// after `bulk_add` to persist all embeddings at once.
    ...
    /// Load all embeddings from this DuckDB. Used when a Rust-written
