...
- **Never hand-edit `CHANGELOG.md`, `plugins/CHANGELOG.md`, or any
  `crates/*/CHANGELOG.md`.** They are generated by release-please from
  the conventional-commit history — a manual entry is stripped on the
  next run and only causes churn. To land a changelog line, write the
  commit message (`feat:` / `fix:`); the entry appears at release time.
