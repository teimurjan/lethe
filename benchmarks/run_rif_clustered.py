"""Clustered RIF: cue-dependent suppression via query clusters.

Tests whether per-cluster suppression outperforms global suppression.
An entry suppressed as a distractor for "travel" queries remains
unsuppressed for "food" queries. Based on the SAM model's cue-dependent
retrieval probability (Raaijmakers & Shiffrin, 1981).

Shares FAISS/BM25/cross-encoder across all configs.
"""
...
DATA = Path("data")
RESULTS = Path("benchmarks/results/BENCHMARKS_RIF_CLUSTERED.md")
...
CONFIGS = [
    ...
    Config("global", RIFConfig(alpha=0.3, suppression_rate=0.1, reinforcement_rate=0.05, decay_lambda=0.005), 0),
    Config("10-clusters", RIFConfig(alpha=0.3, suppression_rate=0.1, reinforcement_rate=0.05, decay_lambda=0.005), 10),
    Config("30-clusters", RIFConfig(alpha=0.3, suppression_rate=0.1, reinforcement_rate=0.05, decay_lambda=0.005), 30),
    Config("50-clusters", RIFConfig(alpha=0.3, suppression_rate=0.1, reinforcement_rate=0.05, decay_lambda=0.005), 50),
    Config("100-clusters", RIFConfig(alpha=0.3, suppression_rate=0.1, reinforcement_rate=0.05, decay_lambda=0.005), 100),
]
...
def main() -> None:
    print("=" * 60)
    print("Clustered RIF: cue-dependent suppression")
    print("=" * 60)
    print()
    ...
    # Build query clusters for each config that needs them
    ...
    for c in CONFIGS:
        label = "global" if c.n_clusters == 0 else f"{c.n_clusters} clusters"
        print(f"  {c.name}: alpha={c.rif.alpha} ({label})")
    print()

    # Per-config state
    # Global configs: dict[str, float]
    # Clustered configs: ClusteredSuppressionState
    ...
    for step in range(BURN_IN):
        ...
        for ci, cfg in enumerate(CONFIGS):
            ...
            # Get suppression scores for this config
            if cfg.n_clusters == 0:
    ...
    print(f"{'Config':<20} | {'NDCG@10':>8} | {'Δ':>7} | {'Recall@30':>9} | {'Δ':>7}")
    print("-" * 65)
    ...
    for ci, cfg in enumerate(CONFIGS):
        ...
        print(f"{cfg.name:<20} | {ndcg:>8.4f} | {d_ndcg:>+6.1f}% | {recall:>9.4f} | {d_recall:>+6.1f}%")
        md_rows.append((cfg.name, cfg.n_clusters, ndcg, d_ndcg, recall, d_recall))
    ...
    with open(RESULTS, "w") as f:
        f.write("# Clustered RIF Benchmark\n\n")
        f.write("Cue-dependent suppression: per (entry, query_cluster) instead of global.\n\n")
        f.write(f"Burn-in: {BURN_IN} steps, Eval: {n_eval} queries\n")
        f.write(f"RIF params: alpha=0.3, rate=0.1, decay=0.005 (conservative, same for all)\n")
        f.write(f"Date: {time.strftime('%Y-%m-%d')}\n\n")
        f.write("| Config | Clusters | NDCG@10 | Δ | Recall@30 | Δ |\n")
        f.write("|--------|----------|---------|---|-----------|---|\n")
        for name, nc, ndcg, d_ndcg, recall, d_recall in md_rows:
            label = "global" if nc == 0 else str(nc)
            f.write(f"| {name} | {label} | {ndcg:.4f} | {d_ndcg:+.1f}% | {recall:.4f} | {d_recall:+.1f}% |\n")
